...
class TestPersistAndGetState:
    ...
    async def test_persist_state_sets_in_flight_status(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
    ...
    async def test_persist_state_sets_started_at(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
    ...
    async def test_persist_state_preserves_existing_status(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
    ...
    async def test_persist_state_preserves_existing_started_at(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
    ...
    async def test_get_state_returns_none_for_unknown_id(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
...
class TestUpdateStepStatus:
    ...
    async def test_update_step_status_creates_steps_dict_if_missing(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
    ...
    async def test_update_step_status_modifies_existing_step(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
...
class TestMarkCompleted:
    ...
    async def test_mark_completed_successful(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
        await adapter.persist_state({"correlation_id": "abc-123"})
...
class TestGetInFlight:
    ...
    async def test_returns_only_in_flight_states(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
    ...
    async def test_returns_empty_list_when_none_in_flight(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
        await adapter.persist_state({"correlation_id": "done-1"})
        await adapter.mark_completed("done-1", successful=True)
...
class TestGetStale:
    ...
    async def test_returns_states_started_before_cutoff(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
        old_time = datetime(2024, 1, 1, tzinfo=timezone.utc)
        recent_time = datetime(2026, 1, 1, tzinfo=timezone.utc)
    ...
    async def test_returns_empty_list_when_no_stale(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
...
class TestCleanup:
    ...
    async def test_removes_old_completed_states(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
        # Create a completed state with old completed_at
        old_completed_at = datetime.now(timezone.utc) - timedelta(hours=5)
    ...
    async def test_cleanup_removes_failed_states_too(
        self, adapter: InMemoryPersistenceAdapter
    ) -> None:
        old_completed_at = datetime.now(timezone.utc) - timedelta(hours=5)
